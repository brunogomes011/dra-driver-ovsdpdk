...
func (r *OvsDpdkResourcePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Fetch the node so we can match NodeSelector terms against its labels.
...
// matchesNodeSelector returns true if the given node labels satisfy the
...
func (r *OvsDpdkResourcePolicyReconciler) matchesNodeSelector(
	...
	for _, term := range nodeSelector.NodeSelectorTerms {
		...
		if err != nil {
			r.log.V(2).Info("Failed to parse NodeSelectorTerm", "error", err, "selector", ls)
			continue
		}
