...
var _ = Describe("PodManager", func() {
	...
	Describe("Get", func() {
		...
		It("should return the stored PreparedDevice and true for a known claim UID", func() {
			...
			pd := &dratypes.PreparedDevice{ClaimUID: uid, ClaimName: "claim-1"}
			pm.Set(uid, pd)
		...
		It("should not remove the entry on Get", func() {
			...
			pd := &dratypes.PreparedDevice{ClaimUID: uid}
			pm.Set(uid, pd)
	...
	Describe("Set", func() {
		It("should overwrite an existing entry", func() {
			...
			pd1 := &dratypes.PreparedDevice{ClaimUID: uid, ClaimName: "first"}
			pd2 := &dratypes.PreparedDevice{ClaimUID: uid, ClaimName: "second"}
			...
			got, found := pm.Get(uid)
			Expect(found).To(BeTrue())
			Expect(got.ClaimName).To(Equal("second"))
		})

		It("should store independent entries for different UIDs", func() {
			uid1 := k8stypes.UID("uid-a")
			uid2 := k8stypes.UID("uid-b")
			pd1 := &dratypes.PreparedDevice{ClaimUID: uid1}
			pd2 := &dratypes.PreparedDevice{ClaimUID: uid2}
	...
	Describe("Delete", func() {
		...
		It("should return the PreparedDevice and remove it from the cache", func() {
			...
			pd := &dratypes.PreparedDevice{ClaimUID: uid, ClaimName: "to-delete"}
			pm.Set(uid, pd)
		...
		It("should return nil on a second delete of the same UID", func() {
			...
			pm.Set(uid, &dratypes.PreparedDevice{ClaimUID: uid})
			pm.Delete(uid)
