...
func (d *Driver) PrepareResourceClaims(ctx context.Context, claims []*resourceapi.ResourceClaim) (map[k8stypes.UID]kubeletplugin.PrepareResult, error) {
	...
	for _, claim := range claims {
		...
		if pd, found := d.podManager.Get(claim.UID); found {
			...
			result[claim.UID] = kubeletplugin.PrepareResult{Devices: pd.Devices}
			continue
		}

		sc, devices, err := d.deviceState.PrepareResourceClaim(ctx, claim)
		if err != nil {
			logger.Error(err, "Failed to prepare claim", "claim", claim.UID)
			result[claim.UID] = kubeletplugin.PrepareResult{Err: err}
			continue
		}
		logger.Info("Prepared claim", "claim", claim.UID, "result", sc, "error", err)

		d.podManager.Set(claim.UID, sc)
		result[claim.UID] = kubeletplugin.PrepareResult{Devices: devices}
		logger.V(1).Info("Prepared claim", "claim", claim.UID)
	}
...
func (d *Driver) UnprepareResourceClaims(ctx context.Context, claims []kubeletplugin.NamespacedObject) (map[k8stypes.UID]error, error) {
	...
	for _, claim := range claims {
		logger.V(1).Info("Unpreparing claim", "claim", claim.UID)

		sc := d.podManager.Delete(claim.UID)
		if sc == nil {
		...
		if err := d.deviceState.UnprepareResourceClaim(ctx, sc); err != nil {
		...
		result[claim.UID] = nil
		logger.V(1).Info("Unprepared claim", "claim", claim.UID)
	}
