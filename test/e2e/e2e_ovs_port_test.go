...
var _ = Describe("Bridge hot-plug", Label("tier1"), func() {
	const (
		bridge     = "br-hotplug"
...
var _ = Describe("Vhost-user port lifecycle", Label("tier1"), func() {
	...
	BeforeEach(func(ctx SpecContext) {
		applyAndCleanup(mustRenderManifest("policy.yaml.tmpl",
			policyData{Name: "e2e-vhost-policy", NodeNames: []string{workers[0]}, Bridges: []string{plat.bridge0}}))
	...
	It("vhost-server-path matches the socket path", func(ctx SpecContext) {
		...
		Expect(err).NotTo(HaveOccurred())
		wantDir := socketDirPath(string(pod.UID), claimName, "vhost-port")
...
var _ = Describe("Port already gone before deletion", Label("tier1"), func() {
	const (
...
var _ = Describe("Two claims get distinct ports — different nodes", Label("troubleshoot"), func() {
	const (
	...
	It("each node has exactly one port on its local bridge", func(ctx SpecContext) {
		...
		applyAndCleanup(mustRenderManifest("pod.yaml.tmpl",
			podData{Name: pod0, Namespace: testNamespace, ClaimName: claim0, NodeName: workers[0]}))
		applyAndCleanup(mustRenderManifest("pod.yaml.tmpl",
			podData{Name: pod1, Namespace: testNamespace, ClaimName: claim1, NodeName: workers[1]}))
